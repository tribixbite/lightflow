...
import { 
  Search, Copy, ExternalLink, Terminal, Code, Package, Database,
  Server, Layout, Palette, Key, Shield, Activity, TestTube,
  Cloud, Sigma, Settings, Star, GitCommit, Clock
} from 'lucide-react';
import {
  Select,
  SelectContent,
...
const jsStackOptions = {
    ...
    bundler: ['Vite', 'Webpack', 'Turbopack', 'Rollup', 'esbuild', 'Parcel', 'SWC', 'tsup'],
    orm: ['Prisma', 'TypeORM', 'Sequelize', 'Mongoose', 'Drizzle', 'Knex', 'MikroORM'],
    backend: ['Express', 'NestJS', 'Fastify', 'Koa', 'Hono', 'Elysia', 'tRPC', 'GraphQL'],
    frontend: ['React', 'Vue', 'Angular', 'Svelte', 'Solid', 'Next.js', 'Nuxt', 'Remix', 'Astro', 'Qwik'],
    uilibrary: ['Tailwind', 'MUI', 'Chakra UI', 'Mantine', 'shadcn/ui', 'DaisyUI', 'Radix UI', 'HeadlessUI', 'Framer Motion'],
    auth: ['NextAuth', 'Auth.js', 'Passport', 'Firebase Auth', 'Supabase Auth', 'Clerk', 'Lucia', 'Kinde'],
    validation: ['Zod', 'Yup', 'Joi', 'class-validator', 'Valibot', 'ValidateJS', 'Superstruct'],
    statemgmt: ['Redux', 'Zustand', 'Jotai', 'XState', 'MobX', 'Recoil', 'TanStack Query', 'Valtio', 'Nanostores'],
    testing: ['Jest', 'Vitest', 'Cypress', 'Playwright', 'RTL', 'MSW', 'Testing Library', 'Storybook'],
    deployment: ['Vercel', 'Netlify', 'Railway', 'Fly.io', 'AWS', 'GCP', 'DigitalOcean', 'Cloudflare'],
    database: ['MongoDB', 'PostgreSQL', 'MySQL', 'Redis', 'SQLite', 'Supabase', 'PlanetScale', 'CockroachDB'],
  };
  ...
  const pythonStackOptions = {
    ...
    bundler: ['Poetry', 'setuptools', 'pip-tools', 'Hatch', 'PDM', 'Maturin', 'Flit'],
    orm: ['SQLAlchemy', 'Django ORM', 'Peewee', 'Tortoise-ORM', 'Pony ORM', 'SQLModel', 'Piccolo'],
    backend: ['FastAPI', 'Django', 'Flask', 'Starlette', 'aiohttp', 'Pyramid', 'Sanic', 'Litestar'],
    frontend: ['Django Templates', 'Jinja2', 'HTMX', 'Alpine.js', 'Streamlit', 'Dash', 'Panel', 'Gradio'],
    uilibrary: ['TailwindCSS', 'Bootstrap', 'Bulma', 'DaisyUI', 'PyWebIO', 'Nicegui', 'Flet', 'Reflex'],
    auth: ['Django Auth', 'FastAPI Users', 'Authlib', 'Python-JOSE', 'PyJWT', 'Starlette Auth', 'Guardian'],
    validation: ['Pydantic', 'Marshmallow', 'Django Validators', 'Cerberus', 'WTForms', 'Schematics', 'voluptuous'],
    statemgmt: ['Django Signals', 'SQLAlchemy Events', 'RxPY', 'PyPubSub', 'Strawberry GraphQL', 'FastAPI DEP', 'Redis PubSub'],
    testing: ['Pytest', 'Unittest', 'Hypothesis', 'Robot Framework', 'Behave', 'Locust', 'Selenium', 'Playwright'],
    deployment: ['Gunicorn', 'Uvicorn', 'Docker', 'AWS Lambda', 'Google Cloud Run', 'Heroku', 'DigitalOcean App Platform'],
    database: ['PostgreSQL', 'MySQL', 'MongoDB', 'Redis', 'SQLite', 'Cassandra', 'TimescaleDB', 'InfluxDB'],
  };
...
export const GithubParam = () => {
  ...
  useEffect(() => {
    if (quickSearch) {
      ...
      const results = Object.entries(stackOptions).flatMap(([category, options]) =>
        options
          .filter(option => 
            option.toLowerCase().replace(/[\s.]+/g, '').includes(normalizedSearch)
          )
  ...
  useEffect(() => {
    const baseUrl = new URL('https://github.com/search?type=Repositories&utf8=%E2%9C%93&fork=true&archived=false');
    let queryString = '';

    // Add global filters
    if (globalFilters.stars) {
      baseUrl.searchParams.set('stars', `>${globalFilters.stars}`);
    }
    if (globalFilters.commits) {
      baseUrl.searchParams.set('commits', `>${globalFilters.commits}`);
    }
    if (globalFilters.updatedWithin) {
      const date = new Date();
      date.setDate(date.getDate() - parseInt(globalFilters.updatedWithin));
      const formattedDate = date.toISOString().split('T')[0];
      baseUrl.searchParams.set('pushed', `>${formattedDate}`);
    }
      ...
      <CardContent className="space-y-6 pt-6">
        ...
        <div className="grid grid-cols-4 gap-4 p-4 rounded-md border bg-black/20">
          <div className="space-y-2">
            <label className={`text-sm font-medium ${theme.label} flex items-center gap-2`}>
            ...
            <Input
              ...
              onChange={(e) => setGlobalFilters(prev => ({ ...prev, stars: e.target.value }))}
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => setGlobalFilters(prev => ({ ...prev, commits: e.target.value }))}
              className={theme.input}
            />
          </div>
          <div className="space-y-2 col-span-2">
            ...
            <Select
              ...
              onValueChange={(value) => setGlobalFilters(prev => ({ ...prev, updatedWithin: value }))}
            >
        ...
        <Popover open={isQuickSearchOpen} onOpenChange={setIsQuickSearchOpen}>
          ...
          <PopoverContent className={`w-[calc(100%-3rem)] ${theme.selectContent}`} align="start">
            <Command>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {Object.entries(stackOptions).map(([category, options]) => (
            <div key={category} className="space-y-2">
              <label className={`text-sm font-medium capitalize ${theme.label} flex items-center gap-2`}>
        ...
        <div className={`flex flex-wrap gap-2 min-h-12 p-4 rounded-md border ${theme.badgeContainer}`}>
        ...
        <div className="flex items-center gap-4 p-4 w-full max-w-full">
          <div className={`w-full max-w-full flex-1 relative p-3 rounded-md border font-mono text-sm ${theme.urlContainer}`}>
